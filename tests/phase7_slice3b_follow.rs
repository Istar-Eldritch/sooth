...
#[test]
fn narrowed_guard_keeps_branch_and_tag_located() {
    // R4: the two consumers this slice does *not* deliver keep their located
    ...
    // ("`branch` is not permitted on a quotation literal"), not to `unknown
    ...
    //
    // `tag`'s arm is reached the same way `branch`'s is: the guard is
    // name-based, so an untagged quotation literal is enough to reach it.
    //
    // `call` is no longer in this loop: P7.S3d delivered it (it splices a
    // quotation *literal* in place instead of erroring; own coverage lives in
    // `tests/phase7_slice3d.rs`), so it no longer reaches this guard at all.
    //
    ...
    for (name, body) in [
        ...
        assert!(
            ...
            "`{name}`'s rejection must say it names no follow-up slice yet: {err}"
        );
    }
}
