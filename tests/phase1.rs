...
fn enum_declared_then_clause_word_defined_and_eliminated_on_later_lines() {
    let out = run_session(&[
        "type: Shape | Circle r f64 | Rect w f64 h f64 ;",
        ": area ( Shape -- f64 ) | Circle dup mul 3.14159 mul | Rect | w h | w h mul ;",
        "2.0 Circle",
        "area .",
    ]);
    let lines: Vec<&str> = out.lines().collect();
...
fn vm_dogfood_runs_in_repl() {
    let out = run_session(&[
        ...
        ": run ( Vm Op -- i64 ) | Push | vm v | vm v vm-push bump-pc fetch Fetched> run | Add | vm | vm vm-pop VmPop> swap vm-pop VmPop> rot add vm-push bump-pc fetch Fetched> run | Sub | vm | vm vm-pop VmPop> swap vm-pop VmPop> rot sub vm-push bump-pc fetch Fetched> run | Mul | vm | vm vm-pop VmPop> swap vm-pop VmPop> rot mul vm-push bump-pc fetch Fetched> run | Load | vm addr | &vm &mem addr &> @ | x | vm x vm-push bump-pc fetch Fetched> run | Store | vm addr | vm vm-pop VmPop> | v x | &!v &!mem addr &!> x ! v bump-pc fetch Fetched> run | Jz | vm target | vm vm-pop VmPop> 0 eq ~[ &!pc target ! ] ~[ bump-pc ] if fetch Fetched> run | Jmp | vm target | vm &!pc target ! fetch Fetched> run | Halt | vm | vm vm-pop VmPop> swap drop ;",
