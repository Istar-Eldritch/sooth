...
: sum-parity ( i64 i64 Parity -- i64 )\n\
  | Even | acc n | n 0 eq ~[ acc ] ~[ acc n add n 1 sub Odd sum-parity ] if\n\
  | Odd  | acc n | n 0 eq ~[ acc ] ~[ acc n add n 1 sub Even sum-parity ] if\n\
;\n\
...
: run-mix ( i64 i64 Step -- i64 )\n\
  | Go   | acc n | n 0 eq ~[ acc n Halt run-mix ] ~[ acc n add n 1 sub Go run-mix ] if\n\
  | Halt | acc n | acc\n\
;\n\
