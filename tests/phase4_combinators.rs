...
fn quotation_type_is_rejected_at_every_audited_position() {
    ...
    // The REPL's own chokepoints (Item 2's `repl_rows`) and the session-
    // bricking regression (Item 1's `item1_rows`) died with the REPL: both
    // tested `repl.rs`'s per-line `check_types`-only path and its registry
    // rollback, retired mechanism with no `run`/`build` counterpart.
}
...
fn stale_phase6_diagnostics_are_reworded() {
    ...
    // The ninth site (R19's residual REPL line) died with the REPL: it
    // checked through `repl.rs`'s own bare-line boundary, which no longer
    // exists.
}
