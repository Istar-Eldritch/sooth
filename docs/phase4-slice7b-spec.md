...
- **R15.** Admission rule: a **four-way classification on capture kind** over `quotation_captures(id)` (D3):
  ...
  2. **Aggregate value, no deriv** — classify the local's binding by current-frame membership (same test as `:6108`/`:5519`, applied to the captured name). Frame-rooted → rejected at word-output (R24), admitted in-frame per R21. Outer-rooted → admitted anywhere.
...
- **R19.** Extend the erased `Slot` with an optional `Copy` `SurvivingCaptureSetId` (id into a side table, mirroring `QuotId`/`Provenance::quotation_captures`), **not** an inline `HashSet` (would break `Slot: Copy`, `check.rs:102`). Entries hold surviving names + root classification + `DerivId`/`owned_root`. **Scalar snapshots are never members.** Set at every boundary for aggregate/borrow captures, forwarded by shuffles.
...
- **R22.** Word-output escape guard: reject a returned quotation (directly or via a returned struct field / array element carrier) whose surviving set includes a **frame-rooted** capture, past-owning-frame. A targeted walk over the surviving set (not `contains_reference`, which is blind to the env). Scalar snapshots absent by construction.
...
| T-repoint | three 7a goldens (`:123`/`:143`/`:160`) re-pointed | run | 1 | R15 rule 1 admits; each adds a `call` and asserts the value |
| T-repoint-join | `capturing_literal_at_join_is_error_naming_7b` (`:231`) | run | 1 | scalar surviving a word-output boundary via inline snapshot; `true` arm `[ x + ]` x=10, called 4 → 14 |
