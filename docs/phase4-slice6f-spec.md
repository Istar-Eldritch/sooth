...
- **D6 — A loop body is not straight-line.** A reference bound outside a loop and used inside is live for the whole body. Originally realized by blanket invocation-scoping (an inherited name was *never* relaxed inside a nested invocation, loop or not); **generalized in review round 3** once `9d9f857` made the reborrow-suspend rule fully depend on this liveness table, turning the blanket's remaining conservatism into real over-rejections (bind a reference outside an `if`/`times`/quotation body, consume it before the nested block, reborrow its root inside — safe, previously rejected). The rule now grants a name into a nested invocation only when the caller first proves no residual use anywhere past the block (`releasable_into`: not referenced in the invocation's own remaining sibling terms, composed with whatever the caller itself was granted); an `if` arm (execute-once) then tracks a granted name with the same fine last-use rule a locally-bound name gets, while a `times`/quotation body (can run more than once or be entered from elsewhere) pins a granted name live for the *whole* body the moment it is used anywhere inside, never releasing it early — this half of D6 is unchanged in substance, just no longer applied to names that could never possibly reach the body at all.
...
- **Q1 — Per-invocation pre-pass, keyed by names bound in that invocation (option c, narrowed).** Compute a `Liveness` over the invocation's own term list; thread `&Liveness` + index `i` through `check_term` into the consumers. Rejected: (a) a `Binding.last_use` field hits an index-space wall across nested invocations and disturbs the stack-half struct (violates D2); (b) re-scanning per query pays repeatedly for cacheable info. Pre-pass is O(subtree) once per invocation. **Correction (review round):** the (b) rationale does not carry over cleanly to `Provenance.quotation_captures`, added later for the quotation-capture fix — that cache memoizes `capture_names` (O(literal body), cheap, computed once per literal at intern time), but the actual per-query work, `capture_alive_names`' fixpoint over `scope.bound`, is recomputed fresh at every call site regardless. The cache is kept because it costs nothing and the association is already computed once anyway, not because it avoids the re-scanning (b) warns about — that cost is paid on every query either way.
  ...
  **Correction from review round 1 (a real regression, not anticipated by this paragraph as first written):** a `Quotation` is not an `If` arm. A quotation *bound* to a local (`[ ... ] | q |`) does not execute at its own literal position; it executes wherever `q` is later used, arbitrarily far away. Treating it like an `If` arm attributed a capture to the literal's index, so the captured place looked dead the moment the literal was written, silently letting a second live `&!` to the same place through. Round 1's fix inferred the binding a quotation belongs to *syntactically* (a literal immediately followed by the `Bind` naming it, taking that bind's last name), with captures recorded into a `captures: HashMap<String, HashSet<String>>` and propagated by a fixpoint over `last_use`.

  **Correction from review round 2: the syntactic inference itself was incomplete, in three ways verified by construction, not by inspection.** A quotation separated from its `Bind` by another value (`[ ... ] 5 | q n |`) was never detected at all. A `Bind` naming two or more quotations at once (`[ A ] [ B ] | qa qb |`) only ever recognized the topmost, missing a capture in `A`. And a conflict arising while the quotation is still **unbound on the stack** (no `Bind` reached yet) had no syntactic hook to attach to in the first place — found during this correction, not anticipated by round 1's own review. All three were rejected before this slice and silently accepted after round 1's fix.

  **The actual fix: stop inferring the association, read it.** The checker already records which quotation a `Slot`/`Binding` carries (`Slot.quot`/`Binding.quot`, both `Option<QuotRef>`); `bound_quotation_name`, `nested_captures`, and the syntactic fixpoint are deleted rather than patched further. `Liveness::scan` reverts `Quotation` to the unconditional `If`-arm treatment (a floor: a capture is at least attributed to its own literal's index). On top of that floor, `capture_alive_names(stack, scope, prov, live, at)` computes, fresh at each query, every name kept alive by a still-live quotation: a quotation sitting on the stack is unconditionally live (nothing has consumed it, same as any other stack-resident value — this is what closes the unbound-on-stack gap), and a quotation held by a scope binding is live if that binding itself is, by the ordinary `!live.dead(...)` rule *or* because the same computation has already found the binding's own name alive via another quotation's capture — a `while`-loop fixpoint over `scope.bound` (a `Vec`, so deterministic; the round-1 fixpoint's `HashMap` iteration was a source of flaky mutation-test results this avoids). A quotation's own free-name set (`capture_names`, walking its body once, tracking in-body shadowing, recursing into nested `If`/`Quotation`) is a pure function of the literal's AST, cached on `Provenance` by `QuotId` at intern time; it does not need the enclosing scope's bound-name set to filter out ordinary word calls, since an over-included non-local name simply never matches anything when `capture_alive_names` intersects it against real bindings. `live_derivs` and `aliasing_origin`'s name loop both gain `|| captured.contains(&b.name)` alongside `!live.dead(...)`.

  **Note on D2.** D2's text ("both stack halves left byte-for-byte") meant: do not *relax* the stack half's existing rule. `capture_alive_names`'s stack-side loop is not a relaxation of that rule — `stack.iter().filter_map(|slot| slot.deriv)` in `live_derivs`, and the stack tail in `aliasing_origin` (`:876`), are untouched, character for character. It is an *addition*: a name that was never previously counted as live at all (because nothing about it was syntactically visible) is now correctly counted, through a separate term chained alongside the untouched stack rule, not a change to it. Stated explicitly so this doesn't read as silently contradicting D2's own text.

  **Generalized in review round 3 (D6):** an outer binding inherited into scope is no longer *unconditionally* absent from a nested invocation's `last_use`. `releasable_into` (computed by the caller at each `if`/`times`/quotation-call recursion site, from `scope.bound`'s position against `base_depth` and `!references(rest, name)`) grants a proven-safe subset in; `Liveness::scan` seeds those names into its own tracking exactly like a name it binds itself, with the `back_edge` flag choosing fine last-use (an `if` arm) or the `IMMORTAL_IN_BODY` sentinel once used anywhere (a `times`/quotation body, so a per-iteration use cannot die before the back-edge). A name the caller does not grant is unaffected: still absent from `last_use`, never relaxed — D6's original conservatism, just scoped to the names that actually need it.
...
- **M4 (D6 loop carve-out).** Make `dead` also relax inherited names → a reference carried into a `times` body and used there (`reference_carried_into_loop_body_stays_live`) goes **red**, or T8 does.
- **M5 (D6 generalization, review round 3).** Force `releasable_into` to always return `HashSet::new()` → exactly the accept tests the generalization exists for go red (`mutable_borrow_dead_before_an_if_arm_is_accepted` and its `times`/projection/two-level/use-then-reborrow/self-tail-loop siblings), nothing else. Force a `times`/`call` recursion's `back_edge` argument to `false` → `mutable_borrow_used_across_a_times_back_edge_is_still_error` and `reference_carried_into_loop_body_stays_live` both go red (same mechanism, two repro shapes). Force `references` to always return `false` → the two residual-use reject tests (`..._used_after_an_if_term_is_still_error`, `..._used_after_two_levels_of_nested_if_arms_is_still_error`) go red. Drop the `idx >= base_depth || outer_releasable.contains(...)` half of `releasable_into`'s filter (grant purely on `!references(rest, ...)`) → the two-level residual-use test goes red, proving the cross-invocation composition (not just single-level `!references`) is load-bearing.
