...
- **Branchy polymorphic bodies (R7, deferred — do not enable piecemeal).** Two gaps sit in the `TermKind::If` arm of `poly_term` and are currently unreachable because they fail safe, but a future slice that enables `if` in polymorphic bodies must fix *both together*. (a) The condition is never popped off the `PolyType` stack: `if`/`else` are both walked over the unmodified stack, so every naturally-typed polymorphic `if` body over-rejects at the residual check: `: pass ( 'T bool -- 'T ) | x b | b if x else x end ;` fails because the body is left holding the un-popped `bool` (residual `bool 'T` vs declared `'T`). (b) `PolyScope`'s move-tracking has no branch-join: the arms are checked over cloned scopes that are discarded, so a local consumed identically on both arms is falsely reported as never-consumed (`: pick ( 'T -- bool 'T ) | x | true if x else x end ;`, currently rejected). (b) is masked today because (a) rejects every such body first. Fixing only (b) still rejects everything; fixing only (a), once conditions are popped, unsoundly under-rejects a leak that lives on one arm. Both land in the same change or neither does.
