...
| **P7** | [Language prerequisites for the stdlib](./P7-language-prereqs.md) | `[L]` — in progress (S3u/S3w parked; S6c brief outstanding; S13 poly.rs ruling landed: 7 bands extracted + core holdout recorded) |
| **P7b** | [Higher-kinded types](./P7b-higher-kinded-types.md) | `[L]` — in progress (S1–S10, S8b, S8c, S11, S6d-PREREQ, S6d (slices as Iterator targets — the shared `Slice[i64]` impl shipped in `core::iterator`, both impls draining one imported protocol; the mutable `!Slice[i64]` target closed by Ruling A), S12, P7b.S13 (the poly→poly cross-call App lift), P7b.S14 (provenance-gates the S9 pre-guard's scope-derived ctor params, closing SOO-58), P7b.S15 (Applicative.pure — return-type-polymorphic construction: the declaration gate admits output-App-headed members, a single explicit instantiation grounds output-side at mono member calls, and the `Applicative` trait + co-located Option/Result/List `pure` impls ship in `core`) landed; follow-ups pending: the instantiation-audit exemption for synthesized bundles (S6d-PREREQ's remaining residual — the DQ4 sentinel-substitution grounding route landed with S6d) and the S6d deferrals (mutable target SOO-45, bound-generic consumers SOO-60 — that lift, landed as P7b.S13; generic-element targets SOO-48)) |
