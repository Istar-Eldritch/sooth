...
| **P7b** | [Higher-kinded types](./P7b-higher-kinded-types.md) | `[L]` — in progress (S1–S10, S8b, S8c, S11, S6d-PREREQ, S6d (slices as Iterator targets — the shared `Slice[i64]` impl shipped in `core::iterator`, both impls draining one imported protocol; the mutable `!Slice[i64]` target closed by Ruling A), S12 landed; follow-ups pending: the instantiation-audit exemption for synthesized bundles (S6d-PREREQ's remaining residual — the DQ4 sentinel-substitution grounding route landed with S6d) and the S6d deferrals (mutable target SOO-45, bound-generic consumers SOO-60, generic-element targets SOO-48)) |
