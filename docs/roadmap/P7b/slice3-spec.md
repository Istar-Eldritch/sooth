...
**Phase 2 outcome (recorded at phase exit).** Phase 2 landed the part of S3-7 that does
not depend on S3-1.c: both candidate-disambiguation `continue`s are no longer silent
(each records its reason, and the empty-`fitting` error names every candidate and why it
failed to ground), and the single-candidate path gained `splice_member_unbound_var_error`.
The deletion mandate itself — removing `splice_member_hkt_error` and the `ground_slots`
single-`Type` grounding closure, replacing them with θ-grounding through the caller — is
**deferred to Phase 3 and carried there**: full θ-grounding of App-headed member slots
needs S3-1.c's caller-space obligation machinery, so the fence survives Phase 2 with a
corrected drift note (it IS reachable today for newly-groundable App-headed member shapes
via S3-6's CtorImage stand-in, where it raises a located hard error; it is not
source-unreachable). Phase 3 deletes both and finishes the ruling; the "narrowed, not
deleted" ruling above is resolved at the same point.
...
2. **An HKT-bounded inline word whose trait has no impl in the program is not checked
   ...
   bound, and — third trigger, added at Phase 2 — a bound all of whose impls arity-mismatch
   the Arrow variable's kind, so `arrow_stand_in` yields no viable stand-in and the
   standalone check is skipped the same way). Checked at its splice sites; never spliced
   means never checked. The no-impl half is **unwitnessable by construction** (no impl
   means no call site can discharge the bound, so the word is never spliced); error golden
   E#1 pins the witnessable no-user-bound half at its first splice site.
...
and its relay (S3-1.e, S3-12), and R1.5's re-siting (S3-1.f) — plus **finishing S3-7**
(deferred from Phase 2, see its outcome note): delete `splice_member_hkt_error` and the
`ground_slots` single-`Type` closure, replacing member-slot grounding with θ-grounding
through the caller.
