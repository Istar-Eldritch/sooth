...
**P7b.S14 — Provenance-gate the S9 pre-guard's scope-derived ctor params.**
...
grounding is the located `own_header_cannot_ground_error` naming the missing
import. The gate omits `foreign_single_candidate_grounding`'s own
ambiguity-count clause — the header itself is never in question on this path,
only the borrowed arguments' provenance is — so
`foreign_single_candidate_grounding`'s own decision logic and diagnostics stay
byte-identical (its only edit is the mechanical helper extraction). Ruling: a
caller with no import of the minter but an explicit local instantiation of its
