...
the S1/S2 split was chosen to avoid. Three ways out, and the spec has to pick one:
   ...
   3. **Give inline test sources a manifest**, generated by the harness. Confines the churn
      to test infrastructure, but every inline source becomes a two-file fixture. Note the
      user-level manifest is *not* an option here: a fixture inheriting a developer's
      global config is a reproducibility hole, and the model forbids it for any file with
      an ancestor manifest anyway.
