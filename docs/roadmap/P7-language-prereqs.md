...
**Depends on S3a (landed), S3b (landed), S3n (landed) — not on S3d or S3f.** `Map['K 'V]`
is still *not* writable, but no longer for a field-shape reason: **P7.S3n** landed the
backing storage itself, so a generic struct whose field is an array of its own type
variable (`keys ['K 8]`, or `slots [Ent['K 'V] 8]`) now declares and instantiates. What
remains are a struct-header **length variable** (`'N`, friction #7) and the `Eq`/`Hash`
bound on `'K` plus a `Default`-style bound for constructing the backing array
(friction #6), this slice's own subject. The array form of
