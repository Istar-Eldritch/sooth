...
- **Top-level dispatch** is one `while` in `parse_bodies`
  ...
  symmetric with `extern:` (`parse_extern_decl`, `src/parser.rs:1452`).
...
- **The `&`/`&!` sigil already means "second-class ref to a named place,"**
  ...
  `ast::rename_call` (`src/ast.rs:1595`, inlining a captured local),
  `resolve::strip_ref_sigil` (`src/resolve.rs:150`, rewriting an
...
- **Data emission precedent:** string literals lower through
  ...
  `push_reference` (`src/ir/func_builder/word_families.rs:26-70`). A static's
...
- The initialiser may be **elided**, meaning the type's zero value
  (`static: COUNT i64 ;` is `0`). This is DESIGN.md's `Preelaborate` tier
  (constants/zero only), which falls straight out of "no comptime interpreter".
- **Scalar types only this slice** (`i64`, `u32`, `bool`, and `str` where its
  ...
  declaration with a located error naming the type, deferred to Phase 9.
...
- `resolve::resolve_modules` (`src/resolve.rs:338`) learns `statics` as a name
  ...
  `strip_ref_sigil`'s fallthrough at `src/resolve.rs:150`, recon 3). A core name
  that is neither a local, a module type/word, nor a module static is left
  untouched, exactly as today.
- `ast::rename_call` (`src/ast.rs:1595`, inlining) needs **no change**: a static
  ref is not a bound local, so it already falls through unchanged. Confirm this
  with a note in the diff, do not edit it.

### R3 — a static-rooted borrow escapes the owned-root scans

A static is never owned, moved, or dropped. Its provenance root is not an owned
local, so the consume / borrow-conflict / disposal scans must skip it, the same
way a reference *parameter*'s reborrow carries no `owned_root`
(`src/check/engine.rs:56`). A static-rooted `Deriv` records the static's name as
its `place` (for conflict diagnostics naming both ends) but reports **no**
`owned_root`, so:

- no "forgotten disposal" surplus-value error can fire for a static ref (it is
  a `&T`/`&!T`, already non-linear);
- the existing "a reference cannot be stored" rule
  (`stored_reference_error`, `src/check/declarations.rs:596`) applies
  **unchanged** — a ref rooted in a static may not be put in a struct field,
  array, cell, or another static, because the rule is keyed on the *type*
  (`contains_reference`), not on what the ref points at. This is decision 3's
  whole claim: every existing borrow rule is type-keyed and so applies verbatim.
...
`capture_names`-style traversal, `src/check/engine.rs:548`). Mode is `w` if any
...
- **Exported word:** the `global:` clause is **mandatory** and must equal the
  ...
  - Clause present but disagreeing -> a **single located-error family**
    ...
    span for a missing entry), covering all three disagreements, each naming the
...
- `src/check/engine.rs` / `src/check/declarations.rs`: static-rooted provenance
  skips the owned-root scans (R3); confirm `stored_reference_error` applies
  unchanged.
...
`&`/`&!` behaves as any other ref-typed value under every existing borrow rule —
no escape, no storage — with no new exception carved for it (R1/R3). With Phase 4
(if retained), an agreeing static-using program builds and runs.
...
- `parse_static_struct_type_is_error` — `static: U Uart ;` is a located error
  naming the type (OQ1 deferral surfaces at the declaration).
...
- `fixpoint_converges_on_mutual_recursion` — `a` calls `b`, `b` calls `a`, each
  names one static; both inferred sets equal the union, and the pass terminates
  (this test **fails to terminate**, not merely errs, if the fixpoint is
  written as unguarded recursion — it is the OQ3 guard's witness).
...
- `exact_match_extra_entry_is_error` — declared `COUNT w`, body never touches
  `COUNT`: assert the exact extra-entry message (exact-not-superset).
...
- `storing_a_static_ref_in_a_field_is_error` — a struct field typed to hold a
  ...
  (R3): the rule is type-keyed. This is a **mutation witness** for R3 — it must
  fail if the static branch accidentally routes around `check_no_stored_references`.
...
- `static_ref_escape_diagnostic` — a program trying to store a static ref;
  asserts the unchanged `stored_reference_error` (Exit: no new exception).
...
```json
{
  "phases": [
    ...
    {
      "phase": 2,
      "focus": "Resolution + borrow-typing: teach resolve_modules the static name category (mangle StaticDecl.name per module, rewrite &NAME/&!NAME static references via strip_ref_sigil's fallthrough, R2); add the static branch to the borrow-typing arm in check/word_families.rs so &STATIC/&!STATIC types as &T/&!T and a scalar static is borrowable (R1); static-rooted provenance skips the owned-root scans while stored_reference_error applies unchanged (R3). Confirm rename_call needs no change (note in diff). Borrow unit tests + the stored-ref mutation witness.",
      "effort": "M",
      "difficulty": "hard"
    },
    {
      "phase": 3,
      "focus": "The global-set analysis in a new src/check/globals.rs: direct-set traversal counting only directly-named statics, recursing if-arms and quotation literals (R4, decision 6); the intra-module call-graph adjacency and the monotone worklist fixpoint to convergence with mode-join (R5, OQ3); the boundary exact-match check with the mandatory-on-export / optional-on-private rule and the single located mismatch-error family covering missing/wrong-mode/extra (R6, decision 7). Wired pre-mangle in assemble_module beside check_exported_signatures. Checker unit tests (incl. the mutual-recursion termination witness) and the exit diagnostic goldens.",
      "effort": "L",
      "difficulty": "hard"
    },
    {
