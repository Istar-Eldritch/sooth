...
Split out from P7.S3e (`docs/roadmap/P7/slice3e-brief.md`) as its own prerequisite:
...
- **New, load-bearing finding not in the original recon: S3a's hard case is
  ...
  `generic-enum-elimination-blocked` gap, P7.S3e/construction territory). A poly
  word can never *produce* a generic monomorph that isn't already present as one
  of its inputs, so S3a's minting wall and S3e's construction gap must be
...
6. ~~Relationship to P7.S3e in this slice~~. The registry-lifetime and
   ...
   consumer pressure, not gate the current slice on it. For now, S3e's array
   `sort` consumer remains entirely independent of this type-system plumbing.

## Out of scope

- Trait bounds (P7.S3e's concern entirely).
