...
**Rule: an owning quotation is rejected in every aggregate position**: struct field, **enum
variant field**, array element, slice element, owned-cell payload, `extern:` boundary and
reference referent. Enum variant fields belong on that list and revision 4 omitted them:
enums do support linear variant fields (`examples/list.sth:9`, `Cons … next ^List`), so a
variant field is a P0-shaped position exactly like a struct field.

**The rule costs no new gate. It is an emergent consequence of two decisions already made,
and the important instruction here is to not write a gate at all.**
`audit_quotation_type_registries` (`src/check/audits.rs:158`) is a single central audit over
exactly these registry positions, and it rejects through
`reject_quotation_type_position` (`:453`), which dispatches on the
**`is_quotation_type` accessor**, with its own doc comment stating why: "Accessor, not a
second arm, so the `~` case cannot be silently dropped." The legal-position carve-outs are
structural on the plain variant alone (`matches!(fty, Type::Quotation(_)) => continue`, at
`:172` for a struct field and `:199` for an array element). So the moment the audit table's
required `is_quotation_type` → `Some` lands for `OwningQuotation`, an owning quotation in
any of these positions falls straight past the carve-out into the existing rejection. The
audit runs on the native `check()` path and at the REPL (`src/repl.rs:1697`, `:1828`), so
the containment holds on both, which matters because REPL-bypassed module checks are a
recurring hazard.

Consequences worth stating plainly:

- **Do not add a struct-field gate.** Widening the carve-out is what would break this, and a
  new `is_copy`-based struct-field gate would be actively harmful: it would reject the
  ordinary linear struct fields the language supports and wants. The correct phase-2 work is
  a **test** that the existing audit rejects an owning field, not new machinery.
- **Array and slice elements both reject, but not by the same gates.**
  `check_no_linear_array_elements` (`src/check/declarations.rs:968-983`) and
  `check_slice_element_gate` (`:1003-1026`) reject any element that is not `is_copy`, which
  the owning marker makes it. Only the *array* position is additionally covered by the
  audit: `audit_quotation_type_registries` sweeps structs, enums, arrays, cells and refs and
  never walks `module.slices`, so `check_slice_element_gate` is the sole gate holding the
  slice position. Measured at phase 2: stub the array gate and the audit still rejects, stub
  the slice gate and `Slice[owning [ -- ]]` reaches `ir_type_of` and ICEs. Any later slice
  that lifts an element gate must respect that asymmetry.
  Revision 4's audit table claimed these positions "admit"; that was wrong and is corrected.
- **`field_is_linear` and `layout_field_is_linear` are deliberately untouched.** With the
  rule in force an owning quotation can never be a field, so neither predicate can ever see
...
| struct field, enum variant field | **reject**, and already automatic once `is_quotation_type` returns `Some`, via `audit_quotation_type_registries`; add no gate |
| array element | **already reject**, via both the audit and the non-`Copy` element gate; no new work |
| slice element | **already reject**, via `check_slice_element_gate` alone (the audit skips `module.slices`); no new work |
...
- **The predicate is `is_copy` AND scalar-represented** (not aggregate-backed). This admits
  ...
  struct and an array are always pointer-backed (`is_aggregate` is unconditionally true for
  `IrType::Struct` and `IrType::Array`), an owned cell is never `Copy`, and an enum is scalar
  exactly when it is payload-free. **So the narrowing admits payload-free enums and nothing
  else**, which is the motivating `bool` case, and rejects `[i64 4]` and every `Copy` struct.
- **Compute the scalar half from the declaration, not from `ir::layout`.** "Scalar-represented"
  is an IR-side concept and this is a checker-side fix, but the precedent already exists and
  should be followed rather than reinvented: slice 10c's `tag` restricts its domain to scalar
  enums using `ctx.enums()[id].variants.iter().all(|v| v.fields.is_empty())`
  (`src/check/word_families.rs:669-672`), and its doc comment states the reason explicitly,
  that the predicate is computed from the declaration "not from `ir::layout`, which is why the
  error is reachable before any lowering runs". Use the same declaration-level test here, for
  the same reason. This also means the scalar half needs `enums` only, which `Ctx` already
  exposes.
...
- The per-site audit above, including **the struct-field rejection** (the containment rule)
  and the confirmation that array and slice elements already reject (by different gates, see
  above).
