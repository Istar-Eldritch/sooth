...
- **Array and slice elements are doubly covered.** Besides the audit,
  `check_no_linear_array_elements` (`src/check/declarations.rs:968-984`) and
  `check_slice_element_gate` (`:1008-1025`) reject any element that is not `is_copy`.
...
| array element, slice element | **already reject**, via the same audit and the non-`Copy` element gates; no new work |
