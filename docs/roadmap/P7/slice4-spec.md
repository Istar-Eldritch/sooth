...
- **R3.** Where more than one target matches a concrete type, the most specific wins.
  ...
  constraints neither subsume the other are incomparable; an unordered candidate set is a
  located error. No tiebreak rule is introduced.
...
- **R7.** Two `impl:` blocks for one trait with structurally-equal (alpha-equivalent)
  ...
  `PolyType`), NOT the whole `ImplTarget`. Do not derive `PartialEq` on `ImplTarget` for
  the duplicate check.
...
- **R10.** (NFR, parity) Every existing concrete `impl:` dispatches byte-for-byte
  ...
  warnings && cargo test` stays green. The overload-overlap path (`polyLet me examine the actual commits to map work areas accurately.# Spec: P7.S4 — Generic `impl:` targets with a specificity chain
