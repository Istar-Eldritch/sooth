...
(`:832-835`), and `size_align`'s `Type::Enum` arm calls
`self.ensure_enum(id.index())` then indexes `self.enum_memo[id.index()]`
(`:636-640`), where `enum_memo` was sized `vec![None; enums.len()]`
...
8. `a_combinator_returning_an_array_of_its_grounded_monomorph_builds_and_runs`
   ...
   `src/ir/layout.rs:636-640` from `ensure_array`, since the live `arrays`
