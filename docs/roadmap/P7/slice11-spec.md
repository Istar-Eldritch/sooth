...
**This risk materialized during implementation.** Seven QBE baselines lost a
`type :arr_N` line each in the registries-local commit:
`tests/qbe_baseline/{array_totals,array_totals_hand,combinator_in_times,
combinator_in_times_hand,filter_while,filter_while_hand,times}.ssa`. Verified
benign by grep: each removed symbol is unreferenced elsewhere in its file, i.e.
a dead, declaration-only array stand-in (case 2 above), not a live shape.
...
Golden 10 was added in phase 2's review fix (R4's struct half had no witness);
its outcome is **measured** against the landed implementation, both live and
under a stubbed struct skip.
...
8. `a_combinator_returning_an_array_of_its_grounded_monomorph_builds_and_does_not_panic`
...
9. `a_standalone_mint_after_an_earlier_check_time_mint_lands_at_the_right_id`
   ...
   *Derived, not measured* (round-2's prototype is discarded). Phase 2 must
   ...
   **Phase 2 finding: confirmed measured, as written.** Built against the
   landed R4 implementation: fails with `` unknown word `Ok` in `main` ``,
   naming `main` and not `wrap`.

10. `a_check_time_struct_monomorphs_constructor_is_absent_from_the_call_site_env`
    -- **added in phase 2's review fix.** R4's *struct* half (the
    `struct_generated_sigs` skip/append) is otherwise unwitnessed: goldens
    1-9 all mint an enum, none a struct, and killing the struct loop
    (`struct_skip = usize::MAX`) left the full suite green (measured). `wrap`
    is `inline`, so this also exercises the splice-time recheck of its body
    in `main` -- the same out-of-scope frozen-call-site gap golden 6 pins for
    enums, just via the struct path:
    ...
    must fail with `` unknown word `Cell` in `main` `` (measured); under a
    stubbed struct skip it moves to `` in `wrap` `` (measured), the same
    live-vs-stubbed discriminator as golden 6.
...
| 9 | Revert R6: pass the live `arrays`/`cells`/`refs`/`slices` to the grounding and body walk | `standalone_grounding_an_array_of_a_monomorph_leaves_the_live_arrays_untouched`, `standalone_grounding_a_cell_through_the_body_leaves_the_live_cells_untouched`, `standalone_grounding_a_borrow_and_a_slice_through_the_body_leaves_the_live_registries_untouched`, `corpus_qbe_stays_byte_identical_to_baseline`, golden 8 (panics at `src/ir/layout.rs:719` during lowering instead of exiting 0 -- golden 8 never calls `hold`, so "stays green" here means "the build completes", not "the program runs") | goldens 1-7, 10 |
| 10 | Drop R4's prefix skip (append the whole `helper(local)` output) | **None -- confirmed, measured survivor (phase 2 finding, ahead of phase 3's formal recipe run).** `enum_generated_sigs_prefix_is_stable_under_extension` stays green (it tests the helper, not the skip). This row's earlier draft predicted golden 6 as the killer, reasoning that every base constructor would be doubled in `local_env` so a body call to any of them becomes ambiguous -- **that prediction is superseded and wrong**: the overload resolver silently picks the first identical candidate rather than raising an ambiguity error, confirmed against a purpose-built duplicate-overload fixture that compiles clean, so golden 6 stays green under this mutation too. Golden 8 was never a candidate witness regardless: `hold`'s own body never names `Ok` (that call lives in the caller's quotation argument, and golden 8's `main` no longer supplies one), so a doubled `local_env` is invisible to it. Phase 3 must still record this in the formal pass/fail matrix as a coverage gap with the resolver behaviour as the reason, not re-derive it from scratch | all ten goldens (confirmed full survivor -- nothing fails; the row's only signal is the coverage gap itself) |
...
```json
{
  "phases": [
    ...
    { "phase": 2, "focus": "R4: word-scoped env clone with the newly flushed monomorphs' struct/enum/variant generated sigs appended tail-only, the prefix length computed by calling each helper on the base slice (never by arithmetic on decl counts), passed to check_word. Tests: enum_generated_sigs_over_an_extended_slice_carries_the_monomorphs_own_id (identity of the output Type, not the name), enum_generated_sigs_prefix_is_stable_under_extension, goldens 1, 6, 7 and 9. Record the measured outcome of golden 9, whose stated result is derived rather than measured (golden 8 has no R4 dependency and is measured in phase 1 instead). Landed. Golden 5 (an_output_only_type_variable_is_still_uncallable) was pulled forward from phase 3 and landed here: a stubbed R4 moves its first assertion's message, so it is an R4 witness and belongs with R4. Plus a review fix: golden 9's outcome is confirmed measured as written; golden 10 added to witness R4's struct half (struct_generated_sigs), which goldens 1-9 leave entirely unwitnessed; mutation rows 7, 8 and 10 corrected against measurement (round-3's named witnesses for 7 and 8 were placebos -- golden 7 stays green under a stubbed R4, golden 6 stays green under mutation 8; the true killers are golden 6 and golden 9 respectively -- and mutation 10 is confirmed to survive).", "effort": "M", "difficulty": "tricky" },
    { "phase": 3, "focus": "Golden 5 (output-only type variable boundary) already landed in phase 2 and is not to be re-added; run the ten-mutation recipe and record the measured pass/fail matrix in its formal form (rows 7, 8, 9 and 10 already have phase-2 measurements to check against, not re-derive), including whether goldens 1 and 2 really do stay green on every mint-path row as predicted; roadmap bookkeeping including the P7.S11-follow frozen-env entry; growth-signal re-run on src/check/poly.rs.", "effort": "S", "difficulty": "standard" }
  ]
}
```
