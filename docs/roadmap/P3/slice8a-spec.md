...
**R2 — What may cross.** The numeric tower (`i64`/`u8`/`usize`/`isize`/`f64`/`f32`/`bool`), `&T` and `&!T`, and `cstr`: types whose machine representation is a scalar or a single opaque `Ptr`. *Amended during implementation:* an earlier draft also listed `str`, which contradicts the list's own criterion — R4 makes a `str` a descriptor handle, not a scalar or a single opaque `Ptr`, so C would receive a pointer to a descriptor rather than a `char*`, matching no C prototype without an invented ABI. Nothing is lost, since R7's `cstr` conversion is total.
...
**R9 — `.` prints both, differently.** `str` via `printf("%.*s", len, ptr)`: the carried length is what bounds the print. `cstr` via `%s`, a length being all it lacks — and so it, like C's `strlen`, stops at the first NUL: a literal with an embedded `\0` diverges between Sooth's counted `len` and either's terminator-bound read, which is reachable (`\0` is a supported escape) and pinned by a golden rather than left a surprise. Two new static format strings. *Noted after review:* neither format appends a trailing newline, unlike `.` on every other type (`$fmt` is `"%ld\n"`). This is slated for resolution by making `.` append no separator for any type, with a newline spelled explicitly by the caller; nothing should be built on the current difference persisting.
...
**R11 — `str` is static-rooted; `cstr` is static-rooted except across a declared foreign boundary.** A `str`'s only constructor is a literal, so it can never dangle. A `cstr`'s constructors are a literal's `cstr` conversion (also static) and a foreign call's return value, where R12's declared-boundary trust governs instead of static-rooting: `extern: getenv ( cstr -- cstr ) "getenv" ;` compiles, runs, and lets a `cstr` holding whatever pointer C chose escape a word. Slicing a heap `^[u8 N]` or local buffer into either type is deferred (DESIGN.md Open/deferred): it is a borrow not spelled `&`, and returning it as a reference is exactly what R3 forbids, that rule standing in for lifetimes. A string type's reference-carrying answer is decided by the `_ =>` fall-through arm of `contains_reference` (src/check.rs:247) — and, for `Copy`, of `is_copy` (src/check.rs:199) — so a borrowed view later must give both predicates an explicit arm rather than being hard-coded anywhere else.
