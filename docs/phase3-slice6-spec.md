...
The *aliasing* side of the same join (R21's identity, not R10's borrow state) is deliberately not symmetric with it. Where only one arm's value is aliased, the merge carries that alias forward: the value really does share storage on that arm's path, so over-approximating keeps the rejection at the borrow, where R21 specifies it and where the diagnostic can name both ends and point at `dup`. Only two arms aliasing *different* regions are rejected at the join, since one slot cannot carry both. Rejecting the join on any disagreement was tried and is too blunt: it rejects a merge of a named local against a fresh value, which `examples/list.sth`'s `build` does without ever taking a borrow.
