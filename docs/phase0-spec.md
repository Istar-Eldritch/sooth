...
Captured from `cargo run -- build examples/gcd.sth` (the driver's temp `out.ssa`;
leading indentation normalized for this doc). Names are `%vN`/`@blkN`/`@start`, and the
`0` literal is an explicit `copy` before the compare — not folded into it; `qbe`
accepts the emitted file. Point-free `gcd` and a named-locals `gcd` lower to identical
IR, which is why there are no explicit shuffle instructions here.
