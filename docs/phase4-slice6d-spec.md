...
**Phase 1 review carry-over for U12.** In `times_saves_and_restores_loop_state`, the `carried_slots`/`back_edges` restore assertions are placebos as extended in Phase 1: both are empty before and after regardless of whether `restore_loop_state` restores them, because `finalize_loop` already empties them and the test has no *outer* loop open across the splice. Only the `header`/`entry_block` assertions currently bite. When Phase 2 extends this unit for the `alloca_home` mutation (the `alloca_home` half will bite correctly, since `finalize_loop` never clears it), also open an outer loop across the mid-body splice with non-empty `carried_slots`/`back_edges` so the carried-slot half of the guard is genuinely exercised, not just the alloca-home half.
